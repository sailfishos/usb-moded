...
// ----------------------------------------------------------------
// wtf
// QUARANTINE static gboolean        usbmoded_charging_fallback_timer_cb   (gpointer data);
...
// QUARANTINE static guint usbmoded_charging_fallback_timer_id = 0;
...
// QUARANTINE /* charging fallback handler */
// QUARANTINE static gboolean usbmoded_charging_fallback_timer_cb(gpointer data)
// QUARANTINE {
// QUARANTINE     (void)data;
// QUARANTINE 
// QUARANTINE     usbmoded_charging_fallback_timer_id = 0;
// QUARANTINE 
// QUARANTINE     /* if a mode has been set we don't want it changed to charging
// QUARANTINE      * after 5 seconds. We set it to ask, so anything different
// QUARANTINE      * means a mode has been set */
// QUARANTINE     if( strcmp(usbmoded_get_usb_mode(), MODE_ASK) )
// QUARANTINE         goto EXIT;
// QUARANTINE 
// QUARANTINE     usbmoded_set_usb_mode(MODE_CHARGING_FALLBACK);
// QUARANTINE     /* since this is the fallback, we keep an indication
// QUARANTINE      * for the UI, as we are not really in charging mode.
// QUARANTINE      */
// QUARANTINE     free(current_mode.internal_mode);
// QUARANTINE     current_mode.internal_mode = strdup(MODE_ASK);
// QUARANTINE     current_mode.data = NULL;
// QUARANTINE     log_info("Falling back on charging mode.\n");
// QUARANTINE 
// QUARANTINE EXIT:
...
CHARGE:
    ...
    current_mode.internal_mode = strdup(mode);
    /* CHARGING_FALLBACK is an internal mode not to be broadcasted outside */
    if(!strcmp(mode, MODE_CHARGING_FALLBACK))
        umdbus_send_state_signal(MODE_CHARGING);
    else
        umdbus_send_state_signal(usbmoded_get_usb_mode());
#endif
...
static void usbmoded_set_usb_disconnected_state(void)
{
// QUARANTINE     /* let usb settle */
// QUARANTINE     usbmoded_sleep(1);
// QUARANTINE     /* only disconnect for real if we are actually still disconnected */
...
// QUARANTINE             /* fallback to charging mode after 3 seconds */
// QUARANTINE             if( usbmoded_charging_fallback_timer_id )
// QUARANTINE                 g_source_remove(usbmoded_charging_fallback_timer_id);
// QUARANTINE             usbmoded_charging_fallback_timer_id =
// QUARANTINE                 g_timeout_add_seconds(3, usbmoded_charging_fallback_timer_cb, NULL);
...
// QUARANTINE         struct mode_list_elem *data;
// QUARANTINE         data = usbmoded_get_usb_mode_data();
// QUARANTINE         usbmoded_set_usb_disconnected_state_silent(data);
